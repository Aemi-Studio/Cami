...
extension Color {
    ...
    func uiColorComponents() -> (hue: CGFloat, saturation: CGFloat, lightness: CGFloat) {
        ...
        #if os(macOS)
        let color = StandardizedColor(self).usingColorSpace(.displayP3) ?? StandardizedColor.white
        #else
        let color = StandardizedColor(self)
        #endif
    ...
    func calculateBestContrast(
        ...
        let backgroundLuminance = StandardizedColor(self).luminance()
        ...
        for candidate in candidateColors {
            let candidateLuminance = StandardizedColor(candidate).luminance()
